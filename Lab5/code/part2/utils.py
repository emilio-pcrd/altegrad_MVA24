...
def normalize_adjacency(A):
    ############## Task 9
    tilde_A = A + sp.identity(A.shape[0])
    tilde_D = np.array(np.sum(tilde_A, axis=1)).flatten()
    print("tilde_D: ", tilde_D.shape)
    tilde_D = np.diag(tilde_D)
    inv_D = np.linalg.inv(tilde_D)

    A_normalized = np.sqrt(inv_D) @ tilde_A @ np.sqrt(inv_D)
