...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        ############## Task 7
        adj_batch = [nx.adjacency_matrix(G_train[j]) for j in range(i, min(i+batch_size, N_train))]
        y_batch = torch.tensor(y_train[i:min(i+batch_size, N_train)], dtype=torch.long).to(device)
        ...
        for j in range(i, min(i+batch_size, N_train)):
            num_nodes = G_train[j].number_of_nodes()
            idx_batch.append(torch.full((num_nodes,), j-i))
            offset += num_nodes
...
for i in range(0, N_test, batch_size):
    ...
    ############## Task 7
    adj_batch = [nx.adjacency_matrix(G_test[j]) for j in range(i, min(i+batch_size, N_test))]
    y_batch = torch.tensor(y_test[i:min(i+batch_size, N_test)], dtype=torch.long).to(device)
    ...
    for j in range(i, min(i+batch_size, N_test)):
        num_nodes = G_test[j].number_of_nodes()
        idx_batch.append(torch.full((num_nodes,), j-i))
        offset += num_nodes
    idx_batch = torch.cat(idx_batch).to(device)
